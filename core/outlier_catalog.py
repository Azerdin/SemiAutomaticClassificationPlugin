...
# so the plugin and the standalone benchmark script run the very same code.
# Interface dependencies are passed as callbacks that default to no-ops.
import os
import tempfile
...
def process_catalog_features(
    ...
    # cleans a set once and skips the cache, the benchmark script reuses it.
...
def apply_removal_to_catalog(
    ...
    # catalog, so the order of the steps below matters. Shared entry point for
    # the plugin and the benchmark script.
...
def _extract_roi_vector(geometry_file, sig_id, temp_path_fn=None):
    ...
    # available only inside QGIS, which would block the benchmark script. One
    # signature is recomputed at a time, so the SQL merge is unnecessary;
