# -*- coding: utf-8 -*-
"""Wyznacza przedziały ufności dokładności i jej przyrostu, średnią makro miary F1 oraz liczność zbioru referencyjnego.

WEJŚCIE:      wyniki_badan/benchmark_results_v2 oraz dane_testowe/referencja2
WYJŚCIE:      wyniki_badan/agregacja_v2/przedzialy_ufnosci.csv oraz podsumowanie_ci.txt
URUCHOMIENIE: python3 badania/wyniki/analiza_przedzialy_ufnosci.py   (liczność referencji wymaga GDAL)
"""
...
RESULTS_DIR = os.environ.get("WYNIKI_DIR", RESULTS_ROOT + "/benchmark_results_v2")
OUT_DIR = os.environ.get("AGREGACJA_DIR", RESULTS_ROOT + "/agregacja_v2")

Z = 1.959963984540054

REFERENCE = DATA_DIR + "/referencja2/referencja_FINAL.shp"
REF_CLASS_FIELD = "moja_ocena"
REQUIRED_PER_CLASS = 75
...
def _load_predictions(accuracy_dir):
    ...
    for path in glob.glob(os.path.join(accuracy_dir, "*_pred.csv")):
        ...
        successes = [int(w["mapa"]) == int(w["referencja"]) for w in rows]
        out[(method, algorithm)] = successes
    return out
...
def reference_counts():
    try:
        ...
        return ["", "LICZNOŚĆ ZBIORU REFERENCYJNEGO", "  pominięto, brak modułu GDAL"]
    if not os.path.exists(REFERENCE):
        return ["", "LICZNOŚĆ ZBIORU REFERENCYJNEGO",
                "  pominięto, brak warstwy %s" % _relative_path(REFERENCE)]
    ogr.UseExceptions()
    source = ogr.Open(REFERENCE)
    layer = source.GetLayer(0)
    counts = {}
    for feature in layer:
        value = feature.GetField(REF_CLASS_FIELD)
        if value is not None:
            counts[int(value)] = counts.get(int(value), 0) + 1
    if not counts:
        return ["", "LICZNOŚĆ ZBIORU REFERENCYJNEGO",
                "  pominięto, pole %s jest puste" % REF_CLASS_FIELD]
    smallest = min(counts.values())
    p = EXPECTED_ACCURACY
    wald = Z * sqrt(p * (1 - p) / smallest)
    _, low, high = wilson(round(p * smallest), smallest)
    lines = ["", "LICZNOŚĆ ZBIORU REFERENCYJNEGO",
             "  warstwa:                    %s" % _relative_path(REFERENCE),
             "  punktów razem:              %d" % sum(counts.values()),
             "  wymóg na klasę:             %d" % REQUIRED_PER_CLASS]
    for cls in sorted(counts):
        lines.append("    klasa %d:                  %4d%s"
                     % (cls, counts[cls],
                        "" if counts[cls] >= REQUIRED_PER_CLASS
                        else "   PONIŻEJ WYMOGU"))
    lines += ["  klasa najmniej liczna:      %d" % smallest,
              "  przy dokładności %.0f%% połowa szerokości przedziału wynosi:"
              % (100 * p),
              "    metoda klasyczna:         %.2f pp" % (100 * wald),
              "    metoda Wilsona:           %.2f pp" % (100 * (high - low) / 2)]
    return lines

def _summarise(rows, source):
    import statistics as st
    excludes = [w for w in rows if w["CI_wyklucza_zero"] == "TAK"]
    positives = [w for w in excludes if w["Delta_pp"] > 0]
    ...
    lines = [
        ...
        "     w tym przyrosty:         %d" % len(positives),
        "     w tym spadki:            %d" % (len(excludes) - len(positives)),
