...
def table_T1(rows, out_dir):
    ...
    for algo in ALGO_ORDER:
        ...
        sig = {s: sum(_sig(r) for r in rows
                      if r["Algorytm"] == algo and r["Zakres"] == s) for s in scopes}
        table.append([
            algo, _mean(cells["roi"]), _mean(cells["class"]),
...
def table_T2(rows, out_dir):
    ...
    for state in ["poprawny", "zbledami"]:
        for scope in ["roi", "class"]:
            ...
            table.append([state, scope, _mean([_f(r["Delta_DA_pp"]) for r in g]),
...
def table_T4(rows, out_dir):
    ...
    for raster in sorted(set(r["Raster"] for r in rows)):
        for scope in ["roi", "class"]:
            ...
            table.append([raster, scope, _mean([_f(r["Delta_DA_pp"]) for r in g]),
                          sum(_sig(r) for r in g), len(g)])
    _write_csv(os.path.join(out_dir, "T4_raster_x_zakres.csv"), header, table)
    return header, table

def _baseline_map(rows):
    """Poziom bazowy nie zależy od zakresu detekcji, ponieważ nic nie jest
    usuwane. Ten sam przypadek jest zatem zmierzony dwukrotnie, raz w każdym
    zakresie, wobec czego zwracana jest średnia z obu pomiarów."""
    powtorzenia = defaultdict(list)
    for r in rows:
        group_key = (r["Wariant"], r["Raster"], r["Stan"], r["Algorytm"])
        powtorzenia[group_key].append(_f(r["DA_bazowa_%"]))
    return {k: _mean(v) for k, v in powtorzenia.items()}
...
def table_baseline_correct_vs_errors(rows, out_dir):
    ...
    table = []
    for a in ALGO_ORDER:
        clean_vals, error_vals, better = [], [], 0
        for v in variants:
            for ra in rasters:
                ...
                if p is None or z is None:
                    continue
                clean_vals.append(p); error_vals.append(z)
                if z > p:
                    better += 1
        table.append([a, _mean(clean_vals), _mean(error_vals),
                      _mean(error_vals) - _mean(clean_vals), better, len(clean_vals)])
...
def _ranking(rows, unit_keys, value_fn):
    ...
    for r in rows:
        units[tuple(r[k] for k in unit_keys)][r["Algorytm"]].append(value_fn(r))
    ranks, vals = defaultdict(list), defaultdict(list)
    for unit_vals in units.values():
        # gdy klucz nie zawiera zakresu, ten sam przypadek jest zmierzony
        # dwukrotnie; brana jest wówczas średnia z powtórzeń
        algo_val = {a: _mean(v) for a, v in unit_vals.items()}
...
def table_pelne(rows, out_dir):
    ...
    csv_rows = [[r["Wariant"], r["Stan"], r["Raster"], r["Zakres"], r["Algorytm"],
                 "%.4f" % _f(r["DA_bazowa_%"]), kap_csv(r),
                 r["Najlepsza_konfiguracja"],
                 "%+.4f" % _f(r["Delta_DA_pp"]), r["Istotne_005_lokalnie"]]
                for r in srt]
...
def main():
    ...
    _, full_rows = table_pelne(rows, OUT_DIR)
    print("\nPełna tabela: %d wierszy -> pelne_wyniki_384.csv" % len(full_rows))
