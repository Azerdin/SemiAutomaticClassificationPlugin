...
ROI = sys.argv[1] if len(sys.argv) > 1 else DATA_DIR + "/ROI/warianty/roi_1234567.shp"
CLASS_FIELD = "klasa"
...
def main():
    ...
    n_above_threshold = sum(1 for v, _, _ in pairs if v > JM_THRESHOLD)
    print("\nPar łącznie: %d, powyżej %.2f: %d, minimum: %.2f"
          % (len(pairs), JM_THRESHOLD, n_above_threshold, pairs[0][0]))
