...
DEFAULT_RESULTS_DIR = os.environ.get("WYNIKI_DIR", RESULTS_ROOT + "/benchmark_results_v2")
OUT_DIR = RESULTS_ROOT + "/analiza_wariant_b_v2"
...
def _holm(p_values):
    ...
    for rank_pos, i in enumerate(order):
        value = max(previous, min(1.0, p_values[i] * (m - rank_pos)))
        corrected[i] = value
        previous = value
    return corrected
...
def main():
    ...
    for directory in directories:
        ...
        predictions = _load_predictions(directory)
        if not predictions:
            continue

        tests = []
        for algorithm in ALGORITHMS:
            baseline = predictions.get(("Baseline", algorithm))
            if baseline is None:
                continue
            for (method, alg), successes in predictions.items():
                if alg != algorithm or method == "Baseline":
                    continue
                b, c, p = _mcnemar(baseline, successes)
                tests.append({
                    "Run_ID": run_id, "Algorytm": ALGORITHM_NAME[algorithm],
                    "Metoda": method,
                    "DA_bazowa_%": 100.0 * baseline.mean(),
                    "DA_metody_%": 100.0 * successes.mean(),
                    "Delta_DA_pp": 100.0 * (successes.mean() - baseline.mean()),
                    "b": b, "c": c, "p": p,
                })
        if not tests:
            continue

        corrected = _holm([t["p"] for t in tests])
        for test, p_holm in zip(tests, corrected):
            test["p_Holm_rodzina"] = p_holm
            test["Istotne_005"] = "TAK" if p_holm <= 0.05 else "nie"
            test["Rodzina_n"] = len(tests)
        all_rows.extend(tests)

        for algorithm in ALGORITHMS:
            candidates = [t for t in tests
...
def _save_csv(path, rows):
    if not rows:
        return
    columns = ["Run_ID", "Algorytm", "Metoda", "DA_bazowa_%", "DA_metody_%",
...
def _summary(all_rows, best_rows, source):
    ...
    gains_all = [t for t in significant_all if t["Delta_DA_pp"] > 0]
    significant_best = [t for t in best_rows if t["Istotne_005"] == "TAK"]
    gains_best = [t for t in significant_best if t["Delta_DA_pp"] > 0]
    ...
    lines = [
        ...
        "      w tym poprawy:      %d" % len(gains_all),
        "      w tym pogorszenia:  %d" % (len(significant_all) - len(gains_all)),
        "",
        ...
        "      w tym poprawy:      %d" % len(gains_best),
        "      w tym pogorszenia:  %d" % (len(significant_best) - len(gains_best)),
    ]
    text = "\n".join(lines)
