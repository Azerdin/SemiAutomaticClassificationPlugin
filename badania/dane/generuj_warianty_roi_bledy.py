# -*- coding: utf-8 -*-
"""Wytwarza osiem wariantów zestawu klas z warstwy obszarów treningowych z celowymi błędami.

WEJŚCIE:      dane_testowe/ROI_blad/roi_1234567_bledy.shp
WYJŚCIE:      dane_testowe/ROI_blad/warianty/roi_<wariant>_bledy.shp
URUCHOMIENIE: python3 badania/dane/generuj_warianty_roi_bledy.py   (wymaga GDAL/OGR)
"""
...
SRC = DATA_DIR + "/ROI_blad/roi_1234567_bledy.shp"
OUT_DIR = DATA_DIR + "/ROI_blad/warianty"
PREFIX = "roi_"
SUFFIX = "_bledy"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generuje 8 wariantów zestawu klas z warstwy ROI z~błędami.")
    parser.add_argument(
    ...
    if args.dry_run:
        ...
        print("ROI Z BŁĘDAMI: źródło %s -> %s | pole klasy: %s | %s"
              % (SRC, OUT_DIR, CLASS_FIELD, "OK" if ok else "BRAK PLIKU"))
        print("\n--dry-run: nic nie zapisano.")
        return

    print("=== ROI Z BŁĘDAMI: %s -> %s ===" % (os.path.basename(SRC), OUT_DIR))
