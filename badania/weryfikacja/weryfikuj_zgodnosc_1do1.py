...
def main():
    ...
    from_script = _signatures(script_dir)
    ...
    print("  sygnatur po czyszczeniu:  wtyczka %d, skrypt %d"
          % (len(plugin), len(from_script)))
    print("  usuniętych pikseli (skrypt): %d" % removed)

    common = set(plugin) & set(from_script)
    print("  sygnatur wspólnych (po identyfikatorze): %d" % len(common))
    only_in_plugin = set(plugin) - set(from_script)
    only_in_script = set(from_script) - set(plugin)
    if only_in_plugin:
        print("  tylko we wtyczce: %d" % len(only_in_plugin))
    if only_in_script:
        print("  tylko w skrypcie: %d" % len(only_in_script))

    identical = differing_px = differing_values = 0
    examples = []
    for sig_id in sorted(common):
        kw, pw, ww = plugin[sig_id]
        ks, ps, ws = from_script[sig_id]
    ...
    print("  różna liczba pikseli:            %d" % differing_px)
    print("  różne wartości sygnatury:        %d" % differing_values)
    if examples:
    ...
    if identical == len(common) and not only_in_plugin and not only_in_script:
